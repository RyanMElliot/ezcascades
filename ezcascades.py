...
WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING''' % temp)
    ...
    # compute per-atom potential energy for recording in dump output files
    lmp.command('compute cpe all pe/atom')
    ...
    if not restartfile:
        ...
        if all_input['write_data']:
            lmp.command('write_dump all custom %s/%s/%s.%d.dump id type x y z vx vy vz c_cpe c_cke' % (scrdir, job_name, job_name, iteration))
    ...
    for cloop in range(1, int(1e6)):
        ...
        if ((iteration + cloop) % export_nth) == 0:
            ...
            lmp.command('write_dump all custom %s id type x y z vx vy vz c_cpe c_cke' % dfile)     
